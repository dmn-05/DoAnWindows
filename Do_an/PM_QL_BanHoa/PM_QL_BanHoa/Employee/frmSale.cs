...
namespace PM_QL_BanHoa.Employee {
  public partial class frmSale : Form {
    ...
    private void btnAddToCart_Click(object sender, EventArgs e) {
      ...
      DataTable SanPham = DataProviderBUS.Instance.ExecuteQuery(query, new object[] { productID });
      if (SanPham.Rows.Count == 0) {
        MessageBox.Show("Không tìm thấy sản phẩm!");
        return;
      }
      if (quantity > Convert.ToInt32(SanPham.Rows[0]["SoLuongTonKho"])) {
        ...
        txtQuantity.Text = SanPham.Rows[0]["SoLuongTonKho"].ToString();
        return;
      }
    ...
    private void txtQuantity_KeyPress(object sender, KeyPressEventArgs e) {
      if (e.KeyChar == '-' || e.KeyChar == '.' || e.KeyChar == ',' || (char.IsLetter(e.KeyChar) && !char.IsControl(e.KeyChar))) // nếu là dấu trừ
      {
